...
export function cardFactory(data) {
    const {name, id, time, servings, appliance, ustensils, description, ingredients} = data;
    ...
    function getCardDom() {
        ...
        divCardTitle.classList.add("card-title", "d-flex", "justify-content-between");
    ...
    function getListIngredients() {
        ...
        for(let i = 0; i < ingredients.length; i++) {          
            ...
            if(!quantity && !unit) {
                ...
                ingredientText.push(`<li><strong>${ingredient}:</strong> ${quantity}</li>`)
            } else if(unit) {
                ingredientText.push(`<li><strong>${ingredient}:</strong> ${quantity} ${unit}</li>`)
            }
        }
